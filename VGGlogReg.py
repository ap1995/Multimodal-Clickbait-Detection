...
def run(train_data, valid_data, test_data, truth_data, image_features):
    ...
    train = pd.merge(train, image_features, on = ["id"])
    data = train.values
...
for filename in os.listdir("./media"):
    if filename.endswith(".jpg") or filename.endswith(".png"):
        ...
        image_features.append([pic_id, vgg16_feature])

image_features = pd.DataFrame.from_items(image_features, columns= ["id, postMediaImg"])

run(train_data, valid_data, test_data, truth_data, image_features)