...
def run(train_data, test_data, truth_data):
    ...
    checkpoint = ModelCheckpoint("weights-improvement-{epoch:02d}-{val_acc:.2f}.hdf5") ##ADDED CODE
    callbacks_list = [checkpoint] ###ADDED CODE
    model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['acc'])

    print("model fitting - Bidirectional LSTM")
    model.summary()
    model.fit(x_train, y_train, validation_data=(x_val, y_val), epochs=10, batch_size=50, callbacks=callbacks_list) ##Added code
